...
int consulta_lista_pos(Lista* li, int pos, struct aluno *al){
    if(li == NULL || (*li) == NULL || pos <= 0)
        return 0;
    Elem *no = (*li)->prox;//primeiro elemento
    int i = 1;
    while(no != (*li) && i < pos){
        no = no->prox;
        i++;
...
int insere_lista_ordenada(Lista* li, struct aluno al){
    ...
    if((*li) == NULL){//insere início
        *li = no;
        ...
        if((*li)->dados.matricula <= al.matricula){//insere final
            no->prox = (*li)->prox;
            (*li)->prox = no;
            *li = no;
            return 1;
...
int remove_lista_final(Lista* li){
    ...
    Elem *ant = *li;
    while(ant->prox != (*li))//procura o último
        ant = ant->prox;
...
int remove_lista(Lista* li, int mat){
    ...
    if((*li) == NULL)//lista vazia
        return 0;
    Elem *no = *li;
    if(no->dados.matricula == mat){//remover do final
        if(no == no->prox){//lista fica vaza
            ...
            *li = NULL;
            ...
            Elem *ult = *li;
            while(ult->prox != (*li))//procura o penúltimo
                ult = ult->prox;
            ult->prox = (*li)->prox;
            *li = ult;
            free(no);
